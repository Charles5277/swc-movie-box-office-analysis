import type { WeeklyBoxOffice, MovieRanking, DerivedMetrics, TargetProjection, DataQuality, DailySnapshot } from '~/types'
...
export const taiwanMovieRankings: MovieRanking[] = [
  ...
  { rank: 2, title: '賽德克·巴萊（上）：太陽旗', revenue: 472_650_000, year: 2011, isActive: false },
  ...
  { rank: 8, title: '關於我和鬼變成家人的那件事', revenue: 363_800_000, year: 2023, isActive: false },
...
export function getDecayRate(current: WeeklyBoxOffice, previous: WeeklyBoxOffice | null): number | null {
...
export function calculateTargetProjection(
  ...
  const baseDecayRate = decayRates.length > 0
    ? decayRates.reduce((sum, r) => sum + r, 0) / decayRates.length
    : 0.9
  ...
  // 計算達標所需週均票房（到第 13 週）
  const requiredWeeklyAverage = remainingToTarget > 0 && weeksToProject > 0
    ? remainingToTarget / weeksToProject
    : 0
