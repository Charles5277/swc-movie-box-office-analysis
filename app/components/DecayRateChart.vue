...
const avgDecayRate = chartData.length > 0
  ? chartData.reduce((sum, d) => sum + d.decayRate, 0) / chartData.length
  : 0

// 近 3 週趨勢
const recent3 = chartData.slice(-3)
const recentAvg = recent3.length > 0
  ? recent3.reduce((sum, d) => sum + d.decayRate, 0) / recent3.length
  : 0
  ...
  <UCard class="transition-shadow duration-200 hover:shadow-lg">
    <template #header>
      <div class="flex items-center justify-between">
        ...
        <UBadge :color="trendColor" variant="soft" size="sm">
          近期趨勢：{{ trend }}
        </UBadge>
      </div>
    </template>
    ...
    <template #footer>
      <div class="grid grid-cols-4 gap-3">
        ...
        <div class="p-3 rounded-lg bg-violet-50 dark:bg-violet-900/20 text-center">
          ...
          <p class="font-bold text-violet-700 dark:text-violet-300">{{ avgDecayRate.toFixed(2) }}</p>
        </div>
